...
class valueFirstService {
    ...
    async getTemplates(configData) {
        ...
        try {
            const result = await fetch('https://whatsapp.myvfirst.com/waba/template/fetch?userid=' + configData.WhatsApp_Username + '&pageno=1&pagelimit=200&status=Approved', {
                method: 'GET', headers: {
    ...
    async getToken(configData) {
        try {
            const result = await fetch('https://api.myvfirst.com/psms/api/messages/token?action=generate', {
                method: 'POST', headers: {
                    "Authorization": 'Basic ' + Buffer.from(configData.WhatsApp_Username + ':' + configData.WhatsApp_Password).toString('base64')
                }
            });
    ...
    async sendMessage(token, jsonStr) {
        try {
            const result = await fetch('https://api.myvfirst.com/psms/servlet/psms.JsonEservice', {
